...
* AWS Reserved Instance Optimization
  ...
  2. RIFactor will then match unmatched Reserved Instances together.
     ...
     NOTE: The purpose of joining reserved instances back together
     again would be to optimize applicability. We want, where
     possible, the reserved instances to be in as big as they can be
     so we can apply them to the most nodes we can.  This counteracts
     our tendency to want to split them up again (to fit new node
     sizes.)
