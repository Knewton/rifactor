...
matchReserved = mergeInstances matchFn mergeFn
  where matchFn (Reserved _ r _ _) (Instance _ i) =
        ...
        mergeFn (Reserved e r is nis) o =
          (Reserved e r (o : is) nis)

packReserved :: Transition
packReserved = mergeInstances matchFn mergeFn
  where matchFn (Reserved er r _ _) (Instance ei i) =
          -- windows goes with windows
          (((r ^. ri1ProductDescription) `elem`
            [Just RIPDWindows,Just RIPDWindowsAmazonVPC]) ==
           ((i ^. i1Platform) ==
...
combineReserved (Model os rs cs) =
  ...
  where go [] rs' cs' = Model os rs' cs'
        ...
        matches (Reserved er0 r0 [] _) (Reserved er1 r1 [] _) =
          -- same end date
          (r0 ^. ri1End == r1 ^. ri1End) &&
          -- same instance type group (C1, M3, etc)
          instanceClass (r0 ^. ri1InstanceType ^?! _Just) ==
          instanceClass (r1 ^. ri1InstanceType ^?! _Just) &&
          -- -- same offering type
...
doUpdateReserved r =
  do uuid <- nextRandom
     let r' = r ^. reReserved
         (newFactor,new) =
           foldl (\(total,xs) os@((Instance _ i):_) ->
                    (total
         ...
         (_,rFactor) = instanceClass oldType
...
comparingReserved (Reserved _ r0 _ _) (Reserved _ r1 _ _) =
...
matchingReserved (Reserved _ r0 _ _) (Reserved _ r1 _ _) =
...
comparingInstance (Instance _ i0) (Instance _ i1) =
...
matchingInstance (Instance _ i0) (Instance _ i1) =
